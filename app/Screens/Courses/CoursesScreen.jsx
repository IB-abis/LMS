...
const CoursesScreen = ({ navigation }) => {
    ...
    const fetchAuxLists = async () => {
      try {
        ...
        const pResp = await fetch(
          "https://lms-api.abisaio.com/api/v1/Program/GetProgramList",
        );
        ...
        const cbResp = await fetch(
          "https://lms-api.abisaio.com/api/v1/Course/GetCourseCreatedByList",
        );
  ...
  const fetchCourses = async (opts = {}) => {
    try {
      ...
      const page = 1;
      const rpp =
        opts.rowsPerPage !== undefined ? opts.rowsPerPage : rowsPerPage;

      // Use overrides from opts first, otherwise fall back to state
      const effectiveStatus =
        opts.status !== undefined ? opts.status : status || "All";
      const effectiveCategory =
        opts.category !== undefined ? opts.category : category || "";
      const effectiveLevel =
        opts.level !== undefined ? opts.level : level || "";
      ...
      const effectiveToDate =
        opts.toDate !== undefined
          ...
          : toDate
            ? formatDate(toDate)
            : "";
      const effectiveGlobalCategory =
        opts.globalCategory !== undefined
          ? opts.globalCategory
          : globalCategory || "";
      ...
      const effectiveUserId =
        opts.userId !== undefined ? opts.userId : storedUserId;
          ...
          <View style={styles.scrollContainer}>
            ...
            <View style={styles.tableContainer}>
              <View style={styles.tableHeader}>
                <Text
                  allowFontScaling={false}
                  style={[styles.headerCell, styles.sNoColumn]}
                >
                  SNo.
                </Text>
                <Text
                  allowFontScaling={false}
                  style={[styles.headerCell, styles.courseNameColumn]}
                >
                  Course Name
                </Text>
                <Text
              ...
              {loading ? (
                ...
                courses.map((course, index) => (
                  <View key={course.id} style={styles.tableRow}>
                    <Text
                      allowFontScaling={false}
                      style={[styles.rowCell, styles.sNoColumn, styles.sNoText]}
                    >
                      {index + 1}.
                    </Text>
                    <Text
                    ...
                    <View
                      style={[styles.statusColumn, { alignItems: "center" }]}
                    >
                      <View
                        style={[
                          styles.statusBadge,
                          { backgroundColor: statusBadgeColor(course.status) },
                        ]}
                      >
                        <Text
                          allowFontScaling={false}
                    ...
                    <View
                      style={[styles.actionColumn, { alignItems: "center" }]}
                    >
                      <TouchableOpacity
                        style={styles.actionButton}
                        onPress={() =>
                          navigation.navigate("ActionviewScreen", { course })
                        }
                      >
