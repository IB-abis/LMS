...
const EAssessmentScreen = ({ route, navigation }) => {
  ...
  const trainingSessionIdFromParams =
    route?.params?.trainingSessionId ?? rawMeta?.trainingSessionID;
    ...
    (async () => {
      try {
        ...
        const normalizedAssessmentId =
          assessmentMeta.assessmentId ?? rawMeta?.assessmentID ?? rawMeta?.id;
        ...
        const remainingAttempts = Number(
          assessmentMeta.remainingAttempts ?? rawMeta?.remainingAttempts ?? -1,
        );
  ...
  const submitAssessment = async () => {
    ...
    const payload = {
      ...
      eLearningCourseId: Number(
        assessmentMeta.eLearningCourseId ??
          rawMeta?.eLearningCourseId ??
          rawMeta?.courseId,
      ),
      assessmentType: String(
        assessmentMeta.assessmentType ?? rawMeta?.assessmentType ?? "",
      ),
