...
//         setFileToView({ uri: fileUrl, type: "pdf" });
//         setViewerOpen(true);
//       } else {
...
//                               <TouchableOpacity
//                                 onPress={() =>
//                                   handleViewFile(content.contentUrl)
//                                 }
...
//                 <WebView
//                   source={{
//                     uri: `https://docs.google.com/gview?embedded=true&url=${fileToView.uri}`,
//                   }}
//                   style={{ flex: 1 }}
//                 />
...
const ExploreMoreScreen = ({ navigation }) => {
  ...
  // ✅ Works in both Expo Go and Production build
  const handleViewFile = async (fileUrl) => {
    console.log("📄 View Requested For:", fileUrl);
    try {
      ...
      // PDF files - Google Docs Viewer (works everywhere)
      if (extension === "pdf") {
        const encodedUrl = encodeURIComponent(fileUrl);
        const googleDocsUrl = `https://docs.google.com/viewer?url=${encodedUrl}&embedded=true`;
        console.log("📄 Opening PDF via Google Docs Viewer");
        setFileToView({ uri: googleDocsUrl, type: "web" });
        setViewerOpen(true);
        return;
      }
      ...
      if (
        ...
        const encodedUrl = encodeURIComponent(fileUrl);
        const officeViewerUrl = `https://view.officeapps.live.com/op/embed.aspx?src=${encodedUrl}`;
        console.log("📄 Opening via Microsoft Office Viewer");
        setFileToView({ uri: officeViewerUrl, type: "web" });
        setViewerOpen(true);
        return;
      }

      // Any other file - try Google Docs viewer
      const encodedUrl = encodeURIComponent(fileUrl);
      const googleDocsUrl = `https://docs.google.com/viewer?url=${encodedUrl}&embedded=true`;
      setFileToView({ uri: googleDocsUrl, type: "web" });
      setViewerOpen(true);
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const response = await fetch(
          `https://lms-api.abisaio.com/api/v1/Course/GetGlobalCourse?UserID=${employeeID}&GlobalCategory=${encodeURIComponent(
            selectedCategory,
          )}`,
        );
      ...
      >
        <View style={styles.fullScreenModal}>
          ...
          <View style={styles.modalHeader}>
            ...
            <Text allowFontScaling={false} style={styles.modalHeaderTitle}>
              {fileToView?.type === "video"
                ? "Video Player"
                : "Document Viewer"}
            </Text>
